...
module.exports = {
  ...
  run: async (client, interaction, options) => {
    ...
    const type = options.getString("type") || "oral";
    ...
    if(JSON.parse(character.blacklist).includes(type) || !JSON.parse(character.whitelist).includes(type))
    ...
    if(JSON.parse(victim.blacklist).includes(type) || !JSON.parse(victim.whitelist).includes(type))
