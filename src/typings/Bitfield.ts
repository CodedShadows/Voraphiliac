/**
  ...
  */
export type BitFieldResolvable = bigint | string | BitField | BitFieldResolvable[];
...
class BitField {
   ...
   */
  any(bit) {
    return (this.bitfield & (this.constructor as typeof BitField).resolve(bit)) !== (this.constructor as typeof BitField).DefaultBit;
  }
   ...
   * @returns {string[]}
   */
  missing(bits, ...hasParams: [unknown]) {
  ...
  add(...bits) {
    ...
    if (Object.isFrozen(this)) return new (this.constructor as typeof BitField)(this.bitfield | total);
    this.bitfield |= total;
    return this;
  }
  ...
  serialize(...hasParams: [unknown]) {
    ...
    for (const [flag, bit] of Object.entries((this.constructor as typeof BitField).Flags)) serialized[flag] = this.has(bit, ...hasParams);
    return serialized;
  }
  ...
  *[Symbol.iterator](...hasParams: [unknown]) {
  ...
  static resolve(bit: BitFieldResolvable): bigint {
    ...
    if (typeof DefaultBit === typeof bit && (bit as bigint) >= DefaultBit) return (bit as bigint);
    if (bit instanceof BitField) return bit.bitfield;
    if (Array.isArray(bit)) return bit.map(p => this.resolve(p)).reduce((prev, p) => prev | p, DefaultBit);
    if (typeof bit === "string") {
      if (typeof this.Flags[bit] !== "undefined") return this.Flags[bit];
      const resolved = BigInt(bit);
      if (typeof DefaultBit === typeof resolved && resolved >= DefaultBit) return resolved;
    }
